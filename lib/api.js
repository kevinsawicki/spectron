...
Api.prototype.addWebContentsApis = function (api) {
  ...
  this.addWebContentsCapturePageSupport();
};
...
Api.prototype.addWebContentsCapturePageSupport = function () {
  const app = this.app;
  const self = this;

  app.client.addCommand('webContents.capturePage', function (rect) {
    return this.executeAsync(
      async function (rect, requireName, done) {
        const args = [];
        if (rect != null) args.push(rect);
        ...
        const image = await webContents.capturePage.apply(webContents, args);
        if (image != null) {
          done(image.toPNG().toString('base64'));
        } else {
          done(image);
  ...
  app.webContents.capturePage = function () {
    return app.client['webContents.capturePage'].apply(app.client, arguments);
  };
};
